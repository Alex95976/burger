...
def get_rsi_data(
    ...
    try:
        ...
        if not klines or len(klines) < 50:
            print(
                f"Warning: Not enough kline data for {symbol} to calculate RSI."
            )
            return None
        ...
        for i in range(len(rsi_series) - 1, 0, -1):
            ...
            if new_status:
                if (
                    not trend_status_history
                    or trend_status_history[0] != new_status
                ):
        ...
        s30u_val = (
            price_history["rsi_30_up"][0]
            if price_history["rsi_30_up"]
            else None
        )
        s70d_val = (
            price_history["rsi_70_down"][0]
            if price_history["rsi_70_down"]
            else None
        )
        s70u_val = (
            price_history["rsi_70_up"][0]
            if price_history["rsi_70_up"]
            else None
        )
        s30d_val = (
            price_history["rsi_30_down"][0]
            if price_history["rsi_30_down"]
            else None
        )

        average_status = (
            (s30u_val + s70d_val) / 2.0
            if s30u_val and s70d_val
            else None
        )
        ...
        return {
            ...
            "cross_history": {
                "s30u": s30u_val,
                "s30u_prev": (
                    price_history["rsi_30_up"][1]
                    if len(price_history["rsi_30_up"]) > 1
                    else None
                ),
                "s30d": s30d_val,
                "s30d_prev": (
                    price_history["rsi_30_down"][1]
                    if len(price_history["rsi_30_down"]) > 1
                    else None
                ),
                "s70u": s70u_val,
                "s70u_prev": (
                    price_history["rsi_70_up"][1]
                    if len(price_history["rsi_70_up"]) > 1
                    else None
                ),
                "s70d": s70d_val,
                "s70d_prev": (
                    price_history["rsi_70_down"][1]
                    if len(price_history["rsi_70_down"]) > 1
                    else None
                ),
            },
            "cross_now": {
